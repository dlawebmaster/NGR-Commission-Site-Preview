...
standalone/           Self-contained copy of every page (see below)
make_standalone.py    Regenerates standalone/ from the linked pages
...
### Deploying pages one at a time

Every page depends on `assets/css/site.css`. Uploading new HTML against a stale
stylesheet breaks the header: the nav gained a seventh item with History, and the
breakpoint that hides it moved from 1060px to 1240px. With the old CSS, the
horizontal nav still appears at 1060px, crushes the wordmark into three or four
wrapped lines and collides with "About Us."

**The one-file fix is uploading `assets/css/site.css`.** That corrects the header
on all eleven pages at once.

If you would rather not touch the shared assets, the `standalone/` folder holds a
self-contained copy of every page. Each one inlines the full stylesheet, its
scripts, the chapter dataset where the locator needs it, and the emblem and
favicons as data URIs, so it renders correctly regardless of what is on the
server. Photographs still load from `assets/img/`, since those are content you
upload anyway. Files run 156 to 215 KB because each carries its own CSS.

Regenerate them with `python3 make_standalone.py` after any change. Switch back
to the linked pages once the shared stylesheet is deployed, so a future palette
change reaches every page at once instead of eleven.

### Chapter portal

The portal lives at `ngr_portal.html` in the site root. It is reached from four
places, so a chapter officer finds it wherever they happen to be:

- the utility bar, as a compact outlined chip (desktop)
- the mobile drawer, as a full-width button and a nav entry
- the footer, under Commission
- the officer sign-in card on `chapters.html`

It is not a masthead button on purpose. That row already carries seven nav items
and two buttons, and a third pushes the wordmark into wrapping again below
1400px. The chip gives the portal button weight without costing the header its
single-row fit.

**The portal page itself is not in this build.** It lives in the
`dlawebmaster/dlaguideright` repository as a file named `ngr_portal`, with no
extension, so GitHub Pages will serve it as a download rather than render it as a
page. Rename it `ngr_portal.html` and drop it in the site root and every link
above resolves. If you rename it to something else, change the four `href`
values in `build.py` and rebuild.

### Training Hub

Linked from the same four places as the portal: the utility bar, the mobile
drawer (nav entry plus a button), the footer under Commission, and the officer
card on `chapters.html`. The target is set once in `build.py`:

```python
PORTAL_URL       = "ngr_portal.html"
TRAINING_HUB_URL = "portal/training-hub.html"
```

Both are **relative on purpose**. The portal HTML you supplied uses root-absolute
paths (`/portal/dashboard.html`, `/img/ngrc-logo.png`). Those resolve to the
domain root, so on a GitHub Pages project site
(`dlawebmaster.github.io/National-Guide-Right-Commission/`) every one of them
404s. They only work if the portal is served from the root of its own domain.
Either move the site to a custom domain, or rewrite those paths as relative
